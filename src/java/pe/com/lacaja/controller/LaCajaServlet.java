 ...
 * @author AlfredoRegis
 */
@WebServlet(name = "LaCajaServlet", urlPatterns = {"/LaCajaServlet"})
public class LaCajaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet LaCajaServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet LaCajaServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean valor = getUsuarioPorNroDoc("","");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean valor = getUsuarioPorNroDoc(nroDoc,psw);


        if (valor) {
            List<Beneficiario> benef = getBeneficiario(nroDoc);
            request.setAttribute("benef", benef);
            RequestDispatcher rd = getServletContext().getRequestDispatcher("/listadoOpciones.jsp");
            rd.forward(request, response);                 
        }else{
            request.setAttribute("valor", valor);
            RequestDispatcher rd = getServletContext().getRequestDispatcher("/index.jsp");
    ...
    private boolean getUsuarioPorNroDoc(java.lang.String arg0, java.lang.String arg1) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        pe.com.lacaja.client.LaCajaService port = service.getLaCajaServiceImplPort();
        return port.getUsuarioPorNroDoc(arg0, arg1);
    }
